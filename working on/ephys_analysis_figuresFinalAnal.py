...
def dmso_control():
    uvFolder =folder + '\\holds_UV'   
    filenames=np.array(os.listdir(uvFolder))
   
    abf = pyabf.ABF(uvFolder + '\\' + filenames[0])
    
    time_UV = abf.sweepX    
    current_UV = abf.sweepY
    voltage_UV = abf.sweepC
    stim_UV = abf.data[2]
    ...
    abf = pyabf.ABF(drugFolder + '\\' + filenames[0])
    
    time_drug = abf.sweepX    
    current_drug = abf.sweepY
    voltage_drug = abf.sweepC
    stim_drug = abf.data[2]
    
    
    endFolder = folder + '\\holds_end'   
    filenames=np.array(os.listdir(endFolder))
    filenames.sort()

    abf = pyabf.ABF(endFolder + '\\' + filenames[0])
    
    time_end = abf.sweepX    
    current_end = abf.sweepY
    voltage_end = abf.sweepC
    
    
    totalTimeSize=len(current_UV)+len(current_drug)+len(current_end)
    time =np.linspace(0,totalTimeSize*0.0001,totalTimeSize)
    
    fullCurrent = np.concatenate((current_UV,current_drug,current_end))
    
    UV_led_pad= np.pad(stim_UV,(0,len(fullCurrent)-len(stim_UV)))
    drug_pad= np.pad(stim_drug,(0,len(fullCurrent)-len(stim_drug)))
    ...
    axes[1].plot(time,fullCurrent,linewidth=2,color='k')
    axes[1].plot([0,0],[-250,-1250],linewidth=4.0,color='k')
    axes[1].plot([0,40],[-1250,-1250],linewidth=4.0,color='k')
    pf.noBorders(axes[0])
    pf.noBorders(axes[1])
    tPlot.set_size_inches(8,4)
    pf.saveFigure(tPlot,folder+'\\'+'figures','timeSeriesExtended_inc-DMSO_0-1per_Cap-washOn_1uM_SB-40seconds_1000pA')
    
    
    steadyStateBaseline=np.mean(fullCurrent[0:440000])
    steadyStateIrrad=np.mean(fullCurrent[460000:1000000])
    steadyStateCap=np.mean(fullCurrent[3400000:3900000])
    ...
    # for controls. baseline + control e.g. UV
    df = pd.read_csv(finalAnalysisFolder + '\\holding_current_values_trans_DMSO-0-1per_CAP-1uM.csv',index_col=0)
    

    baseline = np.array(df['startCurrent'])
    UV_irrad = np.array(df['Irrad'])
    cap = np.array(df['CapWashOn'])
    
    data = np.column_stack((baseline,UV_irrad,cap))
    data = data/1e3
    ...
    plt.tight_layout()
    fig.set_size_inches(4.5,4.5) 
    pf.saveFigure(fig,finalAnalysisFolder + r'\\figures\\','holdingCurrent_incDMSO-0-1per_CAP_1uM_poired_230629')

     # stats
     ttest_results = stats.ttest_rel(data[:,0],data[:,1])
     ttest_results_1 = stats.ttest_rel(data[:,1],data[:,2])
     ...
     df_stats = pd.DataFrame(data=ttest_results)
     df_stats['Irrad_to_cap'] = ttest_results_1
     df_stats.insert(0, "What", ["test stat","p-value"], True)
     df_stats.to_csv(finalAnalysisFolder + r'\\stats_threshold_trans_incDMSO-0-1per_CAP_1uM_230629.csv')
    return
...
def CPZ_control():
    ...
    totalTimeSize=len(current_UV)+len(current_drug)
    time =np.linspace(0,totalTimeSize*0.0001,totalTimeSize)
    
    fullCurrent = np.concatenate((current_UV,current_drug))
    
    UV_led_pad= np.pad(stim_UV,(0,len(fullCurrent)-len(stim_UV)))    
    drug_pad = np.pad(stim_drug, pad_width=(len(stim_UV)), mode='constant')
    drug_pad = drug_pad[0:len(stim_UV) + len(stim_drug)]
    ...
    pf.noBorders(axes[1])
    tPlot.set_size_inches(8,4)
    pf.saveFigure(tPlot,folder+'\\'+'figures','timeSeriesExtended_preInc-CPZ-5uM_Cap-washOn_1uM_SB-40seconds_1000pA')
    
    
    steadyStateBaseline=np.mean(fullCurrent[0:440000])
    steadyStateIrrad=np.mean(fullCurrent[490000:1100000])
    steadyStateCap=np.mean(fullCurrent[3400000:3900000])
    ...
    # for controls. baseline + control e.g. UV
    df = pd.read_csv(finalAnalysisFolder + '\\holding_current_values_trans_preInc-CPZ_irrad.csv',index_col=0)
    ...
    plt.tight_layout()
    fig.set_size_inches(4.5,4.5) 
    pf.saveFigure(fig,finalAnalysisFolder + r'\\figures\\','holdingCurrent_incCPZ-5uM_CAP_1uM_poired_230629')
     ...
     df_stats['Irrad_to_cap'] = ttest_results_1
     df_stats.insert(0, "What", ["test stat","p-value"], True)
     df_stats.to_csv(finalAnalysisFolder + r'\\stats_threshold_trans_incCPZ-5uM_CAP_1uM_230629.csv')
...
def finalNumSpikes():
   ...
   # plt.xlim((0.5,3.1))
    ...
    # this figure looks really bad so not sure if it's even worth pursuing
    # to pad number of spikes for maximum number of current injections
    # plt.plot(np.transpose(lalal[0]))
    # plt.plot(np.transpose(lalal[1]))
    # plt.plot(np.transpose(lalal[2]))
    
    # baselineSpikes_total = np.zeros((3,1)) 
    # for tots in range(len(baselineSpikes)):
    #     baselineSpikes_total[tots] = len(baselineSpikes[tots][0])
    
    # maxCurrentInjection = np.max(baselineSpikes_total)
    
    # baselineSpikes_padded = numpy_fillna(baselineSpikes)
    
    # lalal = []
    # for tots in range(len(baselineSpikes)):
    #     inbetween=baselineSpikes[tots]
    #     lalal.append(inbetween[:,1:len(inbetween[0])])
...
# # hmmmmmmmm
# def threshold_summary(folder,df_summary):
#     path=r'/Users/carmelhowe/Desktop/summary.csv'
...
def alternateFinalFigures():
     folder=r'Z:\Labs\Frank Lab\Carmel\Ephys\Hippo\resultsCapsaicin'
     loc_dataSummary = folder + '\\cap_inputResistance.csv'
     
     df = pd.read_csv(loc_dataSummary)

    #  change Nan to 0. fix it later
     df_drug = df.loc[df['washCap'] != 0]   
     
     baseline_inputRes= np.array(df_drug['Baseline'])
     washon_inputRes = np.array(df_drug['washCap'])
     uv_inputRes = np.array(df_drug['UV'])
     drug_inputRes = np.array(df_drug['Cap'])
     
     data = np.column_stack((baseline_inputRes,washon_inputRes,uv_inputRes,drug_inputRes))
     data=data/1e3
     ...
     bp=plt.boxplot(data,boxprops=boxprops,medianprops=medianprops,flierprops=flierprops, widths = 0.4)
     my_xticks = ['Baseline', 'Wash on', 'UV', 'Capsaicin']
     x=np.array([1,2,3,4])
     plt.xticks(x, my_xticks)
    ...
    # plt.xlim((0.5,3.1))
     ...
     pf.saveFigure(fig,folder,'inputResistance_washonOCT_UV_1uM-cap_paired_230519')
           
     # threshold & spikes
     loc_dataSummary = folder + '\\cap_thresSpikes.csv'
    
     df = pd.read_csv(loc_dataSummary)
    
    #  change Nan to 0. fix it later
     df_drug = df.loc[df['Threshold_washOCTCAP'] != 0]   
     
     baseline_thres= np.array(df_drug['Threshold_Baseline'])
     washon_thres = np.array(df_drug['Threshold_washOCTCAP'])
     uv_thres= np.array(df_drug['Threshold_UV'])
     drug_thres = np.array(df_drug['Threshold_Cap'])
     
     data = np.column_stack((baseline_thres,washon_thres,uv_thres,drug_thres))
     
     # deal with nan
     mask = ~np.isnan(data)
     filtered_data = [d[m] for d, m in zip(data.T, mask.T)]
     
     baseline_spikes= np.array(df_drug['Threshold Baseline'])
     washon_spikes = np.array(df_drug['washCap'])
     uv_spikes= np.array(df_drug['UV'])
     drug_spikes = np.array(df_drug['Cap'])
          
    
     #  RMP
     loc_dataSummary = folder + '\\cap_rmp.csv'
    ...
    #  change Nan to 0. fix it later
     ...
     baseline_rmp= np.array(df_drug['Baseline'])
     washon_rmp = np.array(df_drug['washCap'])
     uv_rmp = np.array(df_drug['UV'])
     drug_rmp = np.array(df_drug['Cap'])
     
     data = np.column_stack((baseline_rmp,washon_rmp,uv_rmp,drug_rmp))
    ...
    # plt.xlim((0.5,3.1))
     plt.tight_layout()
     fig.set_size_inches(3,4.5) 
     pf.saveFigure(fig,folder,'rmp_washonOCT_UV_1uM-cap_paired_230519')
     ...
     df_stats = pd.DataFrame(data=ttest_results)
     df_stats.insert(0, "What", ["test stat","p-value"], True)
     df_stats.to_csv(folder + r'\\stats_RMP_trans_OCT-CAP_notIncubated_5uM_230519.csv')
    
     
     return

    
    def beforeAfterDrugTimeSeries():
        current=output_posSteps[3]
        baseline=current[0]
        senk=current[3]
        time = np.linspace(0,len(current[0,0,:])*1/10000,num=len(current[0,0,:]))    

        fig = plt.figure()
        ax = fig.add_subplot(111)
        plt.plot(time[25000:60000],np.transpose(baseline[15,25000:60000]))
        plt.plot(time[25000:60000],np.transpose(senk[15,25000:60000]-100))
        plt.plot([4.5,4.5],[0,50],linewidth=4.0,color='k')
        plt.plot([4.5,5.5],[0,0],linewidth=4.0,color='k')
...
#     gim=trace
#     #clear('set_crossgi')
#     set_crossgi= [i for i,v in enumerate(gim) if v > -10]
...
#         print("you have spikes")
#         index_shift_posgi=np.zeros((len(set_crossgi)))
#         index_shift_posgi[0]=min(set_crossgi)
         
#         index_shift_neggi=np.zeros((len(set_crossgi)))
#         index_shift_neggi[len(set_crossgi)-1]=max(set_crossgi)
        
#         for i in range(len(set_crossgi)-1):
#               if set_crossgi[i+1] > set_crossgi[i]+1:
#                   index_shift_posgi[i+1]=i
#                   index_shift_neggi[i]=i
    
#         # returns nonzero arguments.
#         posIdx=np.array(np.nonzero(index_shift_posgi)) 
#         set_cross_plusgi = np.squeeze(np.array(set_crossgi)[posIdx.astype(int)])
    
#         negIdx=np.array(np.nonzero(index_shift_neggi)) 
#         set_cross_minusgi = np.squeeze(np.array(set_crossgi)[negIdx.astype(int)])
        
#         nspikes= len(set_cross_plusgi) # Number of pulses, i.e. number of windows.
...
#     else: # if there are no spikes in the trace
#         spikemax=[]
#         firstSpike = 0
#         print('no spikes in trace')